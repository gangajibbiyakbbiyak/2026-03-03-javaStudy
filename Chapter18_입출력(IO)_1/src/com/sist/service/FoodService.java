...
//    		   .collect(Collectors.toList());
   ...
   {
	   return fList.stream().filter(food->food.getType().contains(type)).collect(Collectors.toList());
   }
   public List<FoodVO> food_address_find(String type)
   {
	   return fList.stream().filter(food->food.getAddress().contains(type)).collect(Collectors.toList());
   }
}
